...
bool CDBAdo::SetConnectionString(CString strDSN, CString strUserID, CString strPassword)
{
	CString strTmp;
	strTmp.Format("Provider=MSDASQL.1;Password=%s;Persist Security Info=True;User ID=%s;Data Source=%s",strPassword,strUserID,strDSN);
	m_strConnect = strTmp;
	return true;
}
...
// 		}
// 		return	false;
		VarientToCString(vtFld,strValue);
		return true;
	}
	catch(_com_error& comError)
	{
...
void CDBAdo::VarientToCString(_variant_t var,CString& strValue)  
{  
	_variant_t var_t;  
	_bstr_t bstr_t;  
